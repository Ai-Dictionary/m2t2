...
app.use((req, res, next) => {
    ...
    helmet.contentSecurityPolicy({
        ...
        directives: {
            ...
            "img-src": ["'self'", "data:", "https://avatars.githubusercontent.com", "https://ai-dictionary.github.io", "https://vercel.com", "https://raw.githubusercontent.com", "https://kidkrishkode.github.io", "https://mockmantra.in"],
            "connect-src": [
                "'self'",
                "https://maxcdn.bootstrapcdn.com",
...
app.post('/auth', async (req, res) => {
    ...
    if(String(type).startsWith('student')){
        memory.clusterName = 'student';
        profile_info = await memory.find_profile(email);
    }else if(String(type).startsWith('teacher')){
        memory.clusterName = 'teacher';
        profile_info = await memory.find_profile(email);
    }else if(String(type).startsWith('admin')){
    ...
    if(profile_info?.status!=3 && profile_info && Object.keys(profile_info).length > 0){
        if((profile_info.id === email || profile_info.email === email) && profile_info.pass === password){
            ...
            const tokenPayload = JSON.stringify({ token: security.substitutionEncoder(String(type+'-'+profile_info.email+'-'+expiryTime), 'security') });
...
app.get('/', async (req, res) => {
    ...
    if(token){
        ...
        let [type, email_id, expiry] = encripted_info.split("-");
        if(Date.now() < expiry){
...
app.get('/main', async (req, res) => {
    try{
        ...
        //             return null;
        //         }
                let profile_info = user; //await memory.find_profile(email_id);
                profile_info.bg = hex.generateBGColor(profile_info.name, profile_info.email);
        ...
        //     res.status(401).send("unauthorize user entry not allow");
        // }
    }catch(e){
